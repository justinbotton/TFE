...
class AircraftList:
    ...
    def get_dict_vertical_rate(self, mode_s):
        return self.dict[mode_s].get_vertical_rate()
    ...
    def set_speed_in_dict(self, mode_s, speed, heading, direction, tilt, vertical_rate):
        ...
        self.dict[mode_s].set_vertical_rate(vertical_rate)
...
dict_position = {}
database = OrmSqlite("/var/www/html/database/adsb-utc")
...
while True:
    ...
    if line.startswith("*8d"):
        ...
        if frame.is_consistent():
            ...
            database.insert_into_mode_s(mode_s)
            if not aircraft_dict.get_dict().has_key(mode_s):
                aircraft_dict.add_aircraft(mode_s)
            if type_code == 19:
                speed, heading, direction, tilt, vertical_rate = frame.decode_speed_heading()
                aircraft_dict.set_speed_in_dict(mode_s, speed, heading, direction, tilt, vertical_rate)
                database.insert_into_speed_heading(mode_s, speed, heading, direction, tilt, vertical_rate)
            elif 9 <= type_code <= 18:
